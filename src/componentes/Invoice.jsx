    ...
    html2canvas(invoiceElement).then(async (canvas) => {
      ...
      try {
        await postBill(formData)
          .then(() => {
            window.alert("Factura guardada con éxito");
          })
          .catch((error) => {
            console.error("Ocurrió un error al guardar la factura: ", error);
            window.alert(
              "Ocurrió un error al guardar la factura, vuelva a intentarlo"
            );
          });
      } catch (error) {
